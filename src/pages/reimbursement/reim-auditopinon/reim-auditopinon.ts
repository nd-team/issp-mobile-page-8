...
@IonicPage()
@Component({
  selector: 'page-reim-auditopinon',
  templateUrl: 'reim-auditopinon.html',
})
export class ReimAuditopinonPage {
  ...
  confirm(): void {
    this.paramObj.chargerAuditStatus = this.paramObj.chargerAuditStatus ? '通过' : '不通过';
    this.http.put(APP_URL + 'phoneReimburse/v1/charge/auditRecord', this.paramObj)
      .then(res => {
        let msg: string;
        if (res.code == 0) {
          msg = '此次审核成功!';
